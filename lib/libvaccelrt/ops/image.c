...
#if 0
/* 
 * Profiling is not supported yet in Unikraft
 */
struct vaccel_prof_region image_op_stats =
	VACCEL_PROF_REGION_INIT("vaccel_image_op");
#endif

static int vaccel_image_op(enum vaccel_op_type op_type, struct vaccel_session *sess,
...
{
	//return virtio_image_classification(sess, img, out_text,
